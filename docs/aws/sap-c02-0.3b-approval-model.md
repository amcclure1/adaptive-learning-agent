...
For AI-authored content, approval eligibility additionally requires a completed current exact-target author self-audit, a passing exact-target deterministic report, a completed current independent-AI disposition, no unresolved blocking finding, and reviewer inspection of that evidence. A revised digest must receive a new self-audit, validation, and verification. Derived recommendations require verified and approved exact premises. The verifier cannot be the human approver. A human disagreement with a finding requires an explicit adjudication and reasoning; silence never resolves it. Self-audit is procedural evidence only and never approval.
