...
This decision fixes the bounded format-0.3 contract and E7B pilot constraints. It does not permanently select media types, universal asset limits, or runtime-specific rendering mechanisms for later pack formats.
...
Revisit through a new ADR only if a later pack format demonstrates a need for another media type, different limits, context-specific asset presentations, archives, or a different runtime presentation mechanism. Format 0.3 remains fixed and formats 0.1/0.2 remain protected.
