...
Initial bounds are 16 assets, 2 MiB per asset, 8 MiB total, and 4096×4096 pixels. Validation and study perform no network access. Core code owns validation, installed-path confinement, digest verification, and logical asset resolution; adapters receive no user-controlled arbitrary path.
...
Acceptance requires confirmation of limits, PNG structural checks, duplicate policy, exact digest framing/order, controlled logical references, no conversion, no network, and the negative-test matrix in [the format proposal](../asset-pack-format-proposal.md).
