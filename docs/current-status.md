...
Status: pre-alpha / 0.2.0-alpha.1 and 0.2.1-alpha.1 published / generic 0.3B authoring infrastructure implemented / AWS content gated
Updated: 2026-07-19
...
0.3B AWS sources/claims/lessons/questions/reviews/pack realization, conversational Subject Builder adapter, capability configuration, expansion beyond E1A/E7B, scheduling, mastery, readiness, exam simulation, YAML, archives, export ergonomics, signing, marketplaces, broader Hermes distribution, hosted identity, servers, cloud deployment, and stronger local-process isolation remain deferred. Generic infrastructure exists, but it is not AWS content, an installed pack, or authorization to activate or release. No MCP server, connector, AWS access, or database change was added.
