...
Format 0.2 explicitly excludes assets. Accepted ADRs 0014–0016 establish explicit format 0.3, PNG-only static local assets, and no changes to formats 0.1 or 0.2. The design is accepted but not implemented. See [the detailed candidate and pilot research](amateur-extra-asset-pilot-0.2b.md), [accepted format design](asset-pack-format-proposal.md), and [accessibility policy](asset-accessibility-policy.md).
...
The accepted design defines stable asset ID, normalized relative path, PNG media type, SHA-256, dimensions, source/figure identity, separate figure/accessibility rights, title/caption/alt/fallback, and direct ordered lesson/question references. Exact raw bytes and asset paths participate in a format-0.3 pack digest. Key decisions are:
...
No retained authoritative snapshot or asset should be published until redistribution is explicitly approved. Project policy treats official E7-1 geometry, labels, and question references as NCVEC public-domain pool material, but exact embedded-PNG redistribution still requires human source-mapping and project-policy confirmation. This is not legal advice or formal legal review.
...
1. Confirm the provisional embedded PNG's source mapping, E7-1 identity/fidelity, and redistribution under project policy.
2. Approve alt text, caption, fallback, mappings, and non-leakage across all three questions.
3. Test a public native-image path for pinned Hermes v0.18.2 or record fallback-only behavior for that surface; do not alter Hermes core/configuration.
4. Name independent content, rights, and accessibility reviewers.
5. Explicitly authorize implementation and content import in separate scope.
