...
The official NCVEC page places the public-domain statement, question-pool downloads, figure downloads, and figure errata together. The accepted pilot project-policy interpretation is:
...
- project alt text, caption, fallback, lessons, and explanations are original project prose under their existing original-prose license, CC-BY-4.0, not public-domain by inheritance;
...
The NCVEC statement does not separately enumerate “diagram bytes,” annotations, every container representation, or transformations. The accepted project policy therefore does not claim a formal legal conclusion. Exact embedded-PNG redistribution remains gated on human confirmation of source mapping and project-policy approval. This is not legal advice or formal legal review.

If conversion is later separately authorized, format 0.3 requires a derivation record containing source media type/hash, reproducible process/tool/version/parameters/fonts/environment, output hash, and mandatory fidelity review. No conversion or normalization pipeline is authorized.

## Pack-format and asset model

Format 0.3 is required because strict format 0.2 rejects new fields/files, assets change digest semantics, old loaders cannot safely ignore a required figure, and question presentation changes.
...
The existing ten operations and contract version 0.1 remain. Accepted additive behavior is:
...
The core validates/resolves installed bytes and remains runtime-independent. A logical `ala-pack-asset-v1` reference is produced only by the core, bound to pack ID/version/digest and asset ID, resolvable only inside the installed pack store, and rejected when stale, malformed, mismatched, or outside that store. The Hermes adapter resolves only those references and either presents the exact PNG through a verified public runtime mechanism or uses approved fallback. The skill presents the representation before soliciting an answer, confirms access, preserves text/labels exactly, and never uses model vision to interpret or score.

Current official Hermes [plugin documentation](https://hermes-agent.nousresearch.com/docs/developer-guide/plugins) describes JSON-string handlers. Current official [vision documentation](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/features/vision.md) describes image input and model routing, not a generic custom-tool local-image output API for pinned v0.18.2. Native image presentation is **unverified**. Implementation must test it against v0.18.2. If no public mechanism is verified, the adapter uses approved alt text and terminal fallback, records native rendering as unsupported or unverified, and does not modify Hermes core/configuration. Native rendering unavailability alone does not block the remaining format-0.3 acceptance.

## SQLite impact

No SQLite schema migration is permitted for format 0.3. Existing installed-pack identity/version/path/digest and session/presentation relationships are sufficient. Restart reloads and revalidates the installed format-0.3 pack and reconstructs the descriptor. Assets remain pack files, not blobs or learner state.
...
All three are Accepted. Existing ADRs 0001–0013 remain Accepted and unchanged.

## Unresolved decisions and gates

1. Human confirmation that DOCX `image5.png` is exact E7-1, visually faithful, and approved for redistribution under project policy.
2. Named reviewers and approved final alt text, caption, fallback, mappings, and non-leakage findings.
3. Verification of a public native-image presentation path in pinned Hermes v0.18.2, or recording fallback-only presentation for that surface.
4. Exact implementation module/schema details for the strict 0.3 parser and logical resolver; these may not broaden the accepted model.
5. Fresh authoritative source/errata/hash review immediately before any content import.
